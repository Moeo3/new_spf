...
namespace Data {
    ...
    vector<vector<pair<double, double> > > hist_cum;
}
...
void add_subclusters(vector<vector<int> > &subclusters, 
    ...
    if (candidates.size() < label_size) return;
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    for (int i = 0; i < n; ++ i) {
        auto hist = features2hist(Data::features[i]);
        Data::hist.push_back(hist);
    }
    calculate_cum(Data::hist, Data::hist_cum);
    ...
    for (int i = 0; i < wd_list.size(); ++ i) {
        ...
        for (int j = 0; j < wl_list.size(); ++ j) {
            ...
            add_subclusters(subclusters, Data::features, Data::cum, n, ensemble_size, wd, wl, 4, label_size);
            add_subclusters(subclusters, Data::hist, Data::hist_cum, n, ensemble_size, wd, wl, 4, label_size);
            if (subclusters.size() == 0) continue;
            // Algorithm::subclusters = get_subclusters(Data::features, Data::cum, n, ensemble_size, 
            //                                          wd, wl, 4, label_size);
            // if (Algorithm::subclusters.size() == 0) continue;
            
            auto partvec = get_parts(subclusters, label_size, n);
    ...
    auto partvec = get_parts(clusters, label_size, n);
